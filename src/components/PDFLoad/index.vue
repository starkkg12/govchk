    ...
    <div class="container" v-focus-nav>
        <div class="editor-container">
            ...
            <div class="project-list">
                <table class="error-table" v-if="errorFlag === 'true'">
                    <thead>
                    <tr>
                        <th v-if="errorInfo[0].column2 === '項目レベル'">フィールド</th>
                        <th>エラー理由</th>
                        <th>エラー詳細</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="item in errorInfo" :key="index">
                        <td v-if="item.column2 === '項目レベル'">{{item.column3}}</td>
                        <td>{{item.column4}}</td>
                        <td>{{item.column5}}</td>
                    </tr>
                    </tbody>
                ...
                <table v-if="updateFields.length>0">
                    <thead>
                    <tr>
                        <th width="120px">フィールド</th>
                        <th>値</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="(field, index) in updateFields" :key="index" v-show="field.fieldName !== 'ドキュメント名'">
                        ...
                        <td>
                            ...
                            />
<!--                            <el-date-picker-->
<!--                                    v-if="field.fieldType === 'Date_Time'"-->
<!--                                    type="date"-->
<!--                                    v-model="field.fieldValue"-->
<!--                                    @focus="inputfocus(index)"-->
<!--                                    @change="handleChange"-->
...
<!--                            </el-date-picker>-->
        ...
        <div style="width: 100%; margin-top: 10px;">
            <el-button v-if="props.type === 'edit' && updateFields.length > 0" class="fr" type="primary" @click="submit">保存</el-button>
            <el-button class="fr" v-if="props.type !== 'edit'" @click="cancel">×　閉じる</el-button>
            <el-button class="fr" v-if="props.type === 'edit'" @click="cancel">キャンセル</el-button>
...
<script setup>
    ...
    const submit = () => {
        ...
        updateFields.value.forEach((item) => {
            update.header.push(item.fieldName);
            let fieldValue = item.fieldValue
            if (item.fieldType === 'Date_Time') {
                fieldValue = fieldValue.replace(/-/g, '/');
            }
            update.data.push(fieldValue);
        });
        modifyOcr(update).then(response => {
